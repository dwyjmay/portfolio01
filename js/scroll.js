...
$(document).ready(function(){
	...
	function fixPosition(){
		...
		if(container.is(':visible')) {
			if(!container.hasClass('fixed')) {
				if((holder.offset().top-200) <= $(window).scrollTop()) {
          container.addClass('fixed');
        }
			}
			else {
				if((shim.offset().top-200) > $(window).scrollTop()) {
  ...
  // i.e. how far down viewport is the "eye line"
	...
	function setTriggerPoint(){
		triggerPoint = $(window).height()*0.5;
        /*트리거 이동률 조정 0.5*/
	}
	setTriggerPoint();
    ...
    if(container.is(':visible')) {
			var section = false;
			var sectionIndex = 0;
			var currentPosition = $(window).scrollTop() + triggerPoint*0.6;
            //progressbar  프로그래스바 이동률 조정 0.5
			// dots
...
//                    console.log(sectionTop);
	...
	points.click(function(){
		...
		var targetY = $('h2').eq(sectionIndex).offset().top - (triggerPoint * .58);
